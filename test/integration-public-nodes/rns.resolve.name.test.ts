...
describe('name resolution', () => {
  ...
  describe('should throw an error when invalid ERC165 contract (account address) as reverse resolver', () => {
    test('testnet', async () => {
      const web3 = new Web3(PUBLIC_NODE_TESTNET);
      const rns = new RNS(web3);
      await asyncExpectThrowError(async () => rns.reverse('0x799c63A0bd1FaB0D7E784f35766FB496766BB245'), NO_NAME_RESOLUTION);
    });
  });
  ...
  describe('should throw an error when ERC165 that not support name interface (public resolver) as reverse resolver', () => {
    test('testnet', async () => {
      const web3 = new Web3(PUBLIC_NODE_TESTNET);
      const rns = new RNS(web3);
      await asyncExpectThrowError(async () => rns.reverse('0x1c0884e81161B526f6A4baBC557F97649a2c74CC'), NO_NAME_RESOLUTION);
    });
  });
  ...
  describe('should throw an error when the address has a resolver but no resolution set', () => {
    test('testnet', async () => {
      const web3 = new Web3(PUBLIC_NODE_TESTNET);
      const rns = new RNS(web3);
      await asyncExpectThrowError(async () => rns.reverse('0x0BC10AFD0Fc8344Ac3bBDDEE72221F148Ee0Bc61'), NO_REVERSE_RESOLUTION_SET);
    });
  });
