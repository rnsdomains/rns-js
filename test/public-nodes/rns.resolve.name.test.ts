...
describe.each([
  ...
  test('should resolve an address', async () => {
  ...
  describe('should throw an error when invalid ERC165 contract (account address) as reverse resolver', () => {
    test('testnet', async () => {
      ...
      rns = new RNS(web3);
      await asyncExpectThrowRNSError(() => rns.reverse('0x799c63A0bd1FaB0D7E784f35766FB496766BB245'), NO_NAME_RESOLUTION);
    });
  });
  ...
  test('should throw an error when reverse resolution has not been set', async () => {
